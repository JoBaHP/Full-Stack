//implementation of AngularJS controllers//registerUserController will consume the REST service to perform the POST HTTP call
//an errorMessage is set in $scope to display as an error message returned
//from the POST call to the server on the Register New User page
//successful POST call will redirect to list-all-users
<!DOCTYPE html>
    ...
    var authenticate = function(credentials, callback) {
       var headers = $scope.credentials ? {
        ...
        headers : headers
                 ...
                 if (response.data.name) { $rootScope.authenticated = true;
                        } else {$rootScope.authenticated = false;
                        }callback && callback();
                }, function() {$rootScope.authenticated = false;
                        callback && callback();
                });
        }
        authenticate(); 
...
app.controller('registerUserController', function($scope, $http, $location, $route) {
	$scope.submitUserForm = function() {
		$http({
			...
			$lication.path("/list-all-users/");
			$route.reload();
		}, function(errorResponse) {
			$scope.errorMessage = errorResponse.data.errorMesagge;
		});
...
//listUserController to consume the REST API
